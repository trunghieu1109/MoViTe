...
# title = ["avg_per", "per_confi", "pred_confi", "reward"]

# # file_name = str(time.time)

# # pd.DataFrame([title]).to_csv('./log/confidence/per_pred_reward_' + file_name + '.csv', mode='w', header=False, index=None)
...
def calculate_measures_thread(state_list, ego_state, isNpcVehicle, TTC_list,
                              distance_list, probability_list, mid_point=None, collision_tag_=False):
...
def calculate_metrics(agents, ego):
    ...
    while i < observation_time / time_step:
        ...
        control_msg_old = msg_socket.recv(2048)
            
        control_msg_old = json.loads(control_msg_old.decode("utf-8"))
        
        old_acc = control_msg_old["acceleration"]
        
        sim.run(time_limit=time_step)  # , time_scale=2
        # msg_socket.send(json.dumps(["get_data"]).encode("utf-8"))
        
        msg_socket.send(json.dumps(["get_acc"]).encode("utf-8"))
        control_msg_new = msg_socket.recv(2048)
            
        control_msg_new = json.loads(control_msg_new.decode("utf-8"))
        
        new_acc = control_msg_new["acceleration"]
        
        JERK_list.append(abs(new_acc - old_acc) / 0.5)
...
def load_scene():
    ...
    if road_num == '1':
        # state.transform.position.x = 213.7
        # state.transform.position.y = -5.2
        # state.transform.position.z = -72.1
        # state.transform.rotation.y = 285
    ...
    if road_num == '1':
        requests.post(
            # "http://localhost:8933/LGSVL/SetDestination?des_x=-20.5&des_y=-1.9&des_z=-50.3") 
            # "http://localhost:8933/LGSVL/SetDestination?des_x=144.8&des_y=-4.3&des_z=-57.8")
...
@app.route('/LGSVL/Status/DistanceToObstacle', methods=['GET'])
def get_dto():
    global DTO
    c_dto = DTO
    DTO = 0
...
@app.route('/LGSVL/Status/TimeToCollision', methods=['GET'])
def get_ttc():
    global TTC
    c_ttc = TTC
    TTC = 0
