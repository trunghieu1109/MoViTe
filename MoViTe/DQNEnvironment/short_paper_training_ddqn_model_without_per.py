...
if __name__ == '__main__':
    ...
    if reuse_mem_eps != '':
        ...
        for i in range(0, old_dqn.buffer_memory.real_size):
            b_s = old_memory.state[i].tolist()
            b_a = old_memory.action[i][0].item()
            b_r = old_memory.reward[i].item()
            b_s_ = old_memory.next_state[i].tolist()
            b_done_ = old_memory.done[i].item()
            ...
            # print("New sample data: ", dqn.memory[0])
        ...
        # print("Sample data: ", old_dqn.buffer_memory.state[0].tolist())
