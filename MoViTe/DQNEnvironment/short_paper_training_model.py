...
def execute_action(action_id):
    ...
    return proC_list, DTO_list, ETTC_list, obstacle_uid
...
def calculate_reward(action_id):
    ...
    proC_list, DTO_list, ETTC_list, obstacle_uid = execute_action(action_id)
    ...
    action_reward = collision_reward + DTO_reward * 0.5 + ETTC_reward * 0.5
            
    return observation, action_reward, collision_probability, DTO, ETTC, episode_done, proC_list, DTO_list, ETTC_list, obstacle_uid

title = ["Episode", "Step", "State", "Action", "Reward", "Collision Probability", "Collision Probability List", 
         "Distance To Obstacles", "Estimated Time To Collision", "DTO_list", "ETTC_list", "Action_Description", "Done"]
...
if __name__ == '__main__':
    ...
    folder_name = './model/short_paper_borregasave_road1/'
    ...
    df_title = pd.DataFrame([title])
    file_name = str(int(time.time()))
    log_name = '../ExperimentData/short_paper_borregasave_road' + road_num + '_' + file_name + '.csv'
    ...
    for i_episode in range(int(start_eps), int(end_eps)):
        ...
        while True:
            ...
            # take action
            s_, reward, proC, DTO, ETTC, done, proC_list, DTO_list, ETTC_list, obstacle_uid = calculate_reward(action)
            ...
            print('>>>>>step, action, reward, collision_probability, DTO, ETTC, action_description, done: ', step, action,
                    reward, round(proC, 6), round(DTO, 6), round(ETTC, 6), 
                    "<" + action_description + ">",
                    done)
            pd.DataFrame(
                [[i_episode, step, s, action, reward, proC, proC_list, DTO, ETTC, DTO_list, ETTC_list, action_description, done]]).to_csv(
                log_name,
                mode='a',
