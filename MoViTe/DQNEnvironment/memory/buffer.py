...
class PrioritizedReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        for i in range(0, batch_size):
            ...
            pri = self.tree.get_prio(idx)     
                   
            # tree_idx, priority, sample_idx = self.tree.get(pri)
            # print("Tree_idx", tree_idx, "priority: ", priority, "sample_idx: ", sample_idx)

            priorities[i] = torch.tensor(pri)
        ...
        # print("Tree indx: ", tree_idxs)
        # print("Sample indx: ", sample_idxs)
...
class PrioritizedViolationReplayBuffer:
    def __init__(self, state_size, action_size, violation_size, buffer_size, eps=1e-2, alpha=0.1, beta=0.1):
        ...
        self.alpha = alpha  # determines how much prioritization is used, α = 0 corresponding to the uniform case
        self.beta = beta  # determines the amount of importance-sampling correction, b = 1 fully compensate for the non-uniform probabilities
        self.max_priority = 1000 ** 0.7  # priority for new samples, init as eps

        # transition: state, action, reward, next_state, done
        self.state = torch.empty(buffer_size, state_size, dtype=torch.float)
        self.action = torch.empty(buffer_size, action_size, dtype=torch.float)
        self.reward = torch.empty(buffer_size, violation_size, dtype=torch.float)
        self.next_state = torch.empty(buffer_size, state_size, dtype=torch.float)
        self.done = torch.empty(buffer_size, dtype=torch.int)
