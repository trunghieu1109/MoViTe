...
def get_environment_state():
    ...
    state[7] = a['local_diff']
    state[8] = a['local_angle']
    
    # add perception option
    state[9] = a['dis_diff']
    state[10] = a['theta_diff']
    state[11] = a['vel_diff']
    state[12] = a['size_diff']
    
    # add control option
    state[13] = a['throttle']
    state[14] = a['brake']
    state[15] = a['steering_rate']
    state[16] = a['steering_target']
    state[17] = a['acceleration']
...
if __name__ == '__main__':
    ...
    folder_name = './model/internal_feature_sanfrancisco_road3_standard/'
    ...
    if reuse_mem_eps != '':
        ...
        # print("Eval net: ", dqn.eval_net)
        # print("Target net: ", dqn.target_net)
        ...
        for i in range(old_memory.real_size):
            state_ = old_memory.state[i].tolist()       
            action_ = old_memory.action[i][0].item()
            reward_ = old_memory.reward[i].item()     
            next_state_ = old_memory.next_state[i].tolist()   
            done_ = old_memory.done[i].item()
            
            new_state_ = state_[0:3] + state_[6:10] + state_[13:24]
            new_next_state_ = next_state_[0:3] + next_state_[6:10] + next_state_[13:24]
            
            dqn.store_transition(new_state_, action_, reward_, new_next_state_, done_)
            
        print("New: ", dqn.buffer_memory.real_size, dqn.learn_step_counter, dqn.steps_done)
        # print("Old: ", old_memory.real_size)
        print(dqn.action_chosen_prob)
        print(dqn.npc_action_chosen_prob)
        
        # print(dqn.buffer_memory.state[0].tolist())
        # print(old_memory.state[0].tolist())
