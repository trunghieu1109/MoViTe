...
def metrics_extract(exp_file):
    ...
    count_violation = [0, 0, 0, 0, 0, 0]
    
    for index, row in df.iterrows():
        
        reward = float(row['Violation Rate'])
        done = row['Done']
        action = row['Action']
        cp_list = row['Violation Rate List']
        state = row['State'][1:-1].split(",")[0].split(" ")
        state_ = [s for s in state if s != ""]
        
        vioRate_list = cp_list[1:-1].split(",")
        vioRate_list_ = [s for s in vioRate_list if s != ""]
        # print(vioRate_list)
        ...
        for i in range(0, 6):
            ...
            if float(vioRate_list_[i]) == 1.0 and diff > 2:
                count_violation[i] += 1
            # elif float(vioRate_list_[i]) > 0 and diff > 2 and vioRate_list_[6] == 1:
            #     count_violation[i] += 1
                
        reward = float(vioRate_list_[6])
        reward_violation = 0
        
        for i in range(0, 7):
            # print(float(vioRate_list_[i]))
            if float(vioRate_list_[i]) == 1.0:
    ...
    print(f"Passing: {count_violation[0]}, Lane Changing {count_violation[1]}, Turning: {count_violation[2]}, Braking: {count_violation[3]}, Speeding: {count_violation[4]}, Run on Red Light: {count_violation[5]}")
